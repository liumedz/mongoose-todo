
/**
 * Module dependencies.
 */

var express = require('express');
var mongoose = require( 'mongoose' );

mongoose.connect('mongodb://localhost/express-todo');

var db = require('./db')(mongoose);
var todoRoutes = require('./routes/todo-routes')(db.Todo);
var userRoutes = require('./routes/user-routes')(db.User);

var http = require('http');
var path = require('path');

var app = express();

// all environments
app.set('port', process.env.PORT || 3000);
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'jade');
app.use(express.favicon());
app.use(express.logger('dev'));
app.use(express.json());
app.use(express.urlencoded());
app.use(express.methodOverride());
app.use(app.router);
app.use(require('less-middleware')(path.join(__dirname, 'public')));
app.use(express.static(path.join(__dirname, 'public')));

// development only
if ('development' == app.get('env')) {
  app.use(express.errorHandler());
}

app.get('/', todoRoutes.index);
app.get('/todos', todoRoutes.getAll);
app.get('/todos/:id', todoRoutes.get);
app.post('/todos', todoRoutes.post);
app.put('/todos/:id', todoRoutes.put);
app.delete('/todos/:id', todoRoutes.destroy);


app.get('/users', userRoutes.getAll);
app.get('/users/:id', userRoutes.get);
app.post('/users', userRoutes.post);
app.put('/users/:id', userRoutes.put);
app.delete('/users/:id', userRoutes.destroy);
app.put('/users/:id/todos/:todoId', userRoutes.assignTodo);

http.createServer(app).listen(app.get('port'), function(){
  console.log('Express server listening on port ' + app.get('port'));
});
